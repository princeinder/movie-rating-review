"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
exports.MovieController = void 0;
var movie_model_1 = require("../models/movie.model");
var review_model_1 = require("../models/review.model");
var MovieController = /** @class */ (function () {
    function MovieController() {
    }
    MovieController.prototype.addMovie = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var movieModel, err_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        movieModel = new movie_model_1["default"](req.body);
                        return [4 /*yield*/, movieModel.save(function (err, doc) {
                                return err
                                    ? res.status(401).send(err)
                                    : res.send({ message: "Movie added successfully" });
                            })];
                    case 1:
                        _a.sent();
                        return [3 /*break*/, 3];
                    case 2:
                        err_1 = _a.sent();
                        return [2 /*return*/, res.status(401).send(err_1)];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    MovieController.prototype.getMovie = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var _a, page, limit, sort, err_2;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        _b.trys.push([0, 2, , 3]);
                        _a = req.query, page = _a.page, limit = _a.limit, sort = _a.sort;
                        return [4 /*yield*/, movie_model_1["default"].findById(req.params.id)
                                .populate("reviews", null, null, {
                                limit: limit,
                                skip: parseInt(page) * parseInt(limit) - parseInt(limit),
                                sort: { rating: parseInt(sort) }
                            })
                                .exec(function (err, data) {
                                if (err) {
                                    res.status(401).send(err);
                                }
                                else {
                                    data
                                        ? res.send(data)
                                        : res.status(401).send("Movie with id not found");
                                }
                            })];
                    case 1:
                        _b.sent();
                        return [3 /*break*/, 3];
                    case 2:
                        err_2 = _b.sent();
                        return [2 /*return*/, res.status(401).send(err_2)];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    MovieController.prototype.getMovies = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var _a, page, limit, sort, err_3;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        _b.trys.push([0, 2, , 3]);
                        _a = req.query, page = _a.page, limit = _a.limit, sort = _a.sort;
                        return [4 /*yield*/, movie_model_1["default"].find({})
                                .sort({ title: parseInt(sort) })
                                .limit(parseInt(limit))
                                .skip(parseInt(page) * parseInt(limit) - parseInt(limit))
                                .populate("reviews")
                                .exec(function (err, data) {
                                if (err) {
                                    return res.status(401).send(err);
                                }
                                else {
                                    data ? res.send(data) : res.status(401).send("No Movies found");
                                }
                            })];
                    case 1:
                        _b.sent();
                        return [3 /*break*/, 3];
                    case 2:
                        err_3 = _b.sent();
                        return [2 /*return*/, res.status(401).send(err_3)];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    MovieController.prototype.addReview = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var reviewModel, err_4;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        reviewModel = new review_model_1["default"](req.body);
                        return [4 /*yield*/, reviewModel.save(function (err) {
                                err
                                    ? res.status(401).send({ message: err.message })
                                    : res.send({ message: "Thanks for your review" });
                            })];
                    case 1:
                        _a.sent();
                        return [3 /*break*/, 3];
                    case 2:
                        err_4 = _a.sent();
                        return [2 /*return*/, res.status(401).send(err_4)];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    return MovieController;
}());
exports.MovieController = MovieController;
